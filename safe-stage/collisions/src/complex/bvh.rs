...
use crate::common::{Bounded, Collides, Rotation, Transformation, Translation};
...
impl BvhTree {
    ...
    pub fn concat(self, other: BvhTree) -> BvhTree {
        ...
        let bound = Self::colliders_to_bounding(&[first.clone(), second.clone()]);
        Self::Branch(bound, Some(Box::new(self)), Some(Box::new(other)))
    }
    ...
    fn nodes_collide(a: &Option<Box<BvhTree>>, b: &Option<Box<BvhTree>>) -> bool {
        match (a, b) {
            (Some(a), Some(b)) => a.collides_with(b.as_ref()),
            _ => false,
        }
    }
}
...
collides_group_impl!(BvhTree, BvhTree);
...
impl Translation for BvhTree {
    fn translate(&self, translation: &Vector3) -> Self {
        match self {
            Self::Branch(c, left, right) => Self::Branch(
                c.translate(translation),
                left.as_ref().map(|l| Box::new(l.translate(translation))),
                right.as_ref().map(|r| Box::new(r.translate(translation))),
            ),
            Self::Leaf(c) => Self::Leaf(c.translate(translation)),
        }
    }
}

impl Transformation for BvhTree {
    fn transform(&self, rotation: &Quaternion, pivot: &Vector3, translation: &Vector3) -> Self {
        match self {
            Self::Branch(c, left, right) => Self::Branch(
                c.transform(rotation, pivot, translation),
                left.as_ref()
                    .map(|l| Box::new(l.transform(rotation, pivot, translation))),
                right
                    .as_ref()
                    .map(|r| Box::new(r.transform(rotation, pivot, translation))),
            ),
            Self::Leaf(c) => Self::Leaf(c.transform(rotation, pivot, translation)),
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn translate_leaf() {
        let leaf = BvhTree::Leaf(cbox(1.0, 1.0, 1.0, 2.0, 2.0, 2.0));
        let translated = leaf.translate(&Vector3::new(1.0, 1.0, 1.0));

        assert_vectors(Vector3::new(1.0, 1.0, 1.0), translated.min());
        assert_vectors(Vector3::new(3.0, 3.0, 3.0), translated.max());
    }
    ...
    fn translate_branch() {
        ...
        let translated = branch.translate(&Vector3::new(1.0, 1.0, 1.0));

        assert_vectors(Vector3::new(0.0, 0.0, 0.0), translated.min());
        assert_vectors(Vector3::new(2.0, 2.0, 2.0), translated.max());

        match translated {
            BvhTree::Branch(_, Some(left), Some(right)) => {
                assert_vectors(Vector3::new(2.0, 2.0, 2.0), left.min());
                assert_vectors(Vector3::new(2.0, 2.0, 2.0), left.max());
                assert_vectors(Vector3::new(0.0, 0.0, 0.0), right.min());
                assert_vectors(Vector3::new(0.0, 0.0, 0.0), right.max());
            }
            _ => panic!("Expected branch"),
        }
    }
    ...
    fn tree_concat() {
        ...
        let concatenated = leaf.concat(branch);

        assert_vectors(Vector3::new(-1.0, -1.0, -1.0), concatenated.min());
        assert_vectors(Vector3::new(2.0, 2.0, 2.0), concatenated.max());

        match concatenated {
            BvhTree::Branch(Collider::AlignedBox(_), Some(l), Some(r)) => {
                assert!(matches!(*l, BvhTree::Leaf(Collider::AlignedBox(_))));
                match *r {
                    BvhTree::Branch(Collider::AlignedBox(_), Some(l), Some(r)) => {
                        assert!(matches!(*l, BvhTree::Leaf(Collider::Point(_))));
                        assert!(matches!(*r, BvhTree::Leaf(Collider::Point(_))));
                    }
                    _ => panic!("Expected branch"),
                }
            }
